...
export const useAlert: Hook<AlertHookProps | undefined, AlertReturn>  = ({template, global} = {}) => {
    ...
    const renderAlerts = () => {
        ...
        const Alert = /* template ? template :  */component.getState()
        ReactDOM.render(
            <>
            {
                store.getState().map((alert) => {
                    return <Alert key={alert.id} close={hideAlert(String(alert?.id))} alert={alert}/>
                })
            }
            </>,
    ...
    const show = (alert: Alert) => {
        const id = nanoid(15)
        store.dispatch({ type: AlertTypes.SET_ALERT, payload: {...alert, id}})
        renderAlerts()
        if(alert.hide || alert.hide === undefined){
            setTimeout(()=>{
                hideAlert(id)()
